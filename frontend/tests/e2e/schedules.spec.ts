...
test('cadastra um agendamento novo com foto do documento do motorista (frente e verso)', async ({ page }) => {
  let sentBody = ''
  await mockSchedules(page, {
    onCreate: (route) => {
      ...
      return route.fulfill({
        ...
        body: JSON.stringify({
          ...
          has_driver_document_photo_back: true,
          has_vehicle_document_photo: false,
  ...
  await page
    .getByLabel(/Foto do documento do motorista — frente/)
    .setInputFiles({ name: 'cnh-frente.jpg', mimeType: 'image/jpeg', buffer: Buffer.from('fake-front') })
  await page
    .getByLabel(/Foto do documento do motorista — verso/)
    .setInputFiles({ name: 'cnh-verso.jpg', mimeType: 'image/jpeg', buffer: Buffer.from('fake-back') })
