...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("BRA2E1", None),  # curta
        ("BRA2E190", None),  # longa
        ("1RA2E19", None),  # número onde só cabe letra
        ("BRAXE19", None),  # letra onde só cabe número
        ("bra2e19", None),  # não normalizada
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ("BRA2E19", "BRA2E19", 0),
        ("KLM-4821", "KLM4821", 0),
        ("HJK7L2O", "HJK7L20", 1),  # O lido no lugar de 0 (casos reais do baseline)
        ("STUOD12", "STU0D12", 1),
        ("TUV6W78", "TUV6W78", 0),
        ("JKLGFO1", "JKL6F01", 2),  # G->6 e O->0
        ("8RA2E19", "BRA2E19", 1),  # 8 lido no lugar de B
        ("BR ABC1D23", "ABC1D23", 0),  # lixo antes da placa
...
def test_fifth_character_is_never_changed_because_it_defines_the_format():
    # "ABC1O23": o O na 5ª posição pode ser letra (Mercosul) — não vira 0.
...
def test_rejects_reads_that_need_too_many_corrections():
    # 3 trocas: é mais provável ser lixo do que uma placa mal lida.
